...
def entry(path: Path, relative_to: Path | None = None):
    data = path.read_bytes()
    relative = path.relative_to(ROOT).as_posix().replace(" ", "%20")
    return {"path": path.relative_to(relative_to or path.parents[1]).as_posix(), "url": f"{BASE}/{relative}",
...
for folder in sorted(p for p in EXAMPLES.iterdir() if p.is_dir()):
    ...
    files=[entry(entry_path)] + [entry(p) for p in sorted(folder.glob("*.py")) if p != entry_path]
    lib=folder / "lib"
    ...
    if lib.exists():
        drivers.append({"id": f"{metadata['id']}-driver", "name": f"{metadata['title']} driver", "version": "1.0.0",
                        "description": f"Supporting driver files for {metadata['title']}.",
                        "boardTags": metadata["boardTags"],
                        "files": [entry(p, lib) for p in sorted(lib.rglob("*")) if p.is_file()]})
manifest={"schema":"pulsar.micropython.library/v1", "catalog":{"id":"micropython-examples", "name":"MicroPython Examples", "version":VERSION}, "examples":result, "drivers":drivers}
